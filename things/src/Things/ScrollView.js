...
const  withScroll = (
  { translationY, state: gestureState , velocityY, contentHeight, containerHeight }
) => {
  ...
  const isInBound = and(lessOrEq(state.position, upperBound), greaterOrEq(state.position, lowerBound));
  const config = {
...
export default memo(({ children, translateY, onPull }) => {
  ...
  const { gestureHandler, translationY, state , velocityY } = useMemoOne(() => verticalPanGestureHandler(), []);
  
  useCode(() => block([
    set(translateY, withScroll({
      translationY, state , velocityY, contentHeight, containerHeight
    })),
    cond(
